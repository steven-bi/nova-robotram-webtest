...
def Inv_Dyn2(theta, theta_d, theta_dd, f_external=None):
    ...
    a = np.zeros(6)
    alp = np.zeros(6)
    th[0] = theta[0]; d[0] = 0.1367; a[0] = 0; alp[0] = 0
    th[1] = theta[1]+173.01/180*np.pi; d[1] = 0; a[1] = 0; alp[1] = np.pi/2   
    th[2] = theta[2]-157.51/180*np.pi; d[2] = 0; a[2] =0.3846; alp[2] = 0
    th[3] = theta[3]-15.5/180*np.pi; d[3] = 0; a[3] = 0.3179; alp[3] = 0
    th[4] = theta[4]-np.pi/2; d[4] = 0; a[4] = 0.0715; alp[4] = -np.pi/2
    ...
    pc = [
    np.array([0, 0, 0.0052644]),
    np.array([0.284, -0.008, 0.01]),
    np.array([0.202, 0.020531, 0.00032334]),
    np.array([0.06, 0.057, 0]),
    np.array([-1.7702E-14, 0.004, 0.002]),
