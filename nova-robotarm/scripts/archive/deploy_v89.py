"""Deploy pick-and-place v8.9 - fix gripper never opens.

Root cause identified from video:
  Gripper is CLOSED from the very start and never opens.
  Robot learned to press closed gripper onto object from above.
  This never produces actual grasping force to lift.

Why gripper never opens:
  1. URDF prismatic joint lower limit = 0 (closed), Isaac Lab inits to lower limit
  2. No reward incentivizes opening the gripper
  3. grasp_reward rewards CLOSING near object -> robot stays closed
  4. With gripper always closed, pressing top of object still gets:
     - reaching_fine (EE near object) -> high reward
     - grasp (near AND closed) -> reward fires even pressing from above
  -> Robot found local optimum: closed gripper pressing down, no lifting needed

Fix:
  Set gripper initial joint position = 0.04 (OPEN) in robot init_state.
  Now robot MUST learn to:
    Step 1: approach with open gripper (gets reaching reward)
    Step 2: close gripper when near object (gets grasp reward)
    Step 3: lift (gets velocity + position reward)

  This creates the correct sequential behavior: open -> approach -> close -> lift.

No reward changes from v8.8. The velocity reward structure is correct.
Only change: gripper init_state 0 -> 0.04 (open).
"""
import paramiko
import time
...
# ==============================================================================
# FILE 1: arm_grasp/mdp/rewards.py  (same as v8.8, no changes)
# ==============================================================================
...
def reaching_bonus_open(
    ...
    """Approach bonus gated on gripper open. (kept for reference)"""
    obj: RigidObject = env.scene[object_cfg.name]
    ee_frame: FrameTransformer = env.scene[ee_frame_cfg.name]
...
def grasped_object_upvel_reward(
    ...
    """v8.8: object upward velocity when grasped. Cannot be faked by hover."""
    grasp_score = _compute_grasp_score(
        env, grasp_threshold, open_pos, robot_cfg, object_cfg, ee_frame_cfg
    )
    obj: RigidObject = env.scene[object_cfg.name]
...
# ==============================================================================
PICK_PLACE_CFG_PY = r'''"""Pick-and-Place v8.9: fix gripper starts closed (never opens).

Root cause from video analysis:
  Gripper is CLOSED from episode start and never opens.
  URDF prismatic lower limit = 0 (closed), Isaac Lab defaults to lower limit.
  No reward incentivizes opening -> robot just presses closed gripper onto object.
  Result: no actual grasp, no lifting possible.

Fix: set gripper initial joint position = 0.04 (fully OPEN).
  Now robot starts with open gripper and must learn to:
    1. Approach object with open gripper (reaching rewards guide this)
    2. Close gripper when fingers are around object (grasp reward)
    3. Lift (velocity + position reward)

Reward structure: identical to v8.8 (velocity reward, no grasped_ee_height).
Only change: robot.init_state.joint_pos adds gripper open positions.
"""
...
class PickPlaceSceneCfg(InteractiveSceneCfg):
    ...
    # v8.9 FIX: start with gripper OPEN so robot must learn to close it
    robot.init_state.joint_pos = {
        ...
        "joint_5": 0.0,
        "joint_6": 0.0,
        "gripper_left_joint": 0.04,   # OPEN (max = 0.04)
        "gripper_right_joint": 0.04,  # OPEN (max = 0.04)
    }
...
@configclass
class RewardsCfg:
    """v8.9: same rewards as v8.8, gripper init fixed to OPEN.

    With gripper starting OPEN, robot must learn:
      open -> approach -> close -> lift
    Instead of the degenerate:
      closed -> press down -> stay (v8.8 behavior)
    """
...
class ArmPickPlacePPORunnerCfg(RslRlOnPolicyRunnerCfg):
    """PPO config for pick-and-place v8.9.

    v8.9: gripper starts OPEN. Robot must learn open->approach->close->lift.
    """
    num_steps_per_env = 24
    max_iterations = 5000
    save_interval = 500
    experiment_name = "arm_pick_place_v8_9"
    run_name = ""
    resume = False
    logger = "tensorboard"
    empirical_normalization = True
...
def main():
    print("=" * 60)
    print("  Pick-and-Place v8.9 Deployment")
    print("  Fix: gripper init_state CLOSED -> OPEN")
    print("=" * 60)
    print("\nRoot cause: gripper always closed from start, never opens.")
    print("Fix: robot.init_state.joint_pos gripper = 0.04 (open)")
    print("Now: open -> approach -> close -> lift")
    print()
    ...
    print("\nStopping existing training (v8.8)...")
